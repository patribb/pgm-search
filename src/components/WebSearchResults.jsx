        ...
        {results.items?.map((result) => (
          <div className="mb-8 max-w-xl" key={result.link}>
            <div className="group flex flex-col">
              <Link className="text-sm truncate text-[#EF4C37]" href={result.link}>{result.formattedUrl}</Link>
              <Link className="group-hover:underline decoration-purple-700 text-purple-600 font-bold truncate" href={result.link}>{result.title}</Link>
            </div>
